...
enum EndPoint: Equatable {
    ...
    var endpoint: String {
        ...
        case .getFilterOptions:
          return "/orders/get_filters_options"
        }
    }
    ...
    var httpMethod: HTTPMethod {
        ...
        case .login, .verifyMfa, .resendOtpMfa, .forgotPassword, .changePassword , .verifyOTP, .resetPassword, .updateProfile:
            return .POST
        case .getOrders(page: _, limit: _, isDelivered: _, assignedToMe: _, sortOrder: _, orderBy: _, customer: _, carrier: _,enteredBy: _, dispatcher: _, pickupStart: _, pickupEnd: _, deliveryStart: _, deliveryEnd: _ ,orderType: _, scheduleEarlyDate: _, tsa: _), .getFilterOptions:
            return .GET
        }
    }
